...
extension UIScreen {
    ...
    func setBrightness(_ value: CGFloat, animated: Bool) {
        ...
        _brightessQueue.cancelAllOperations()

        let step: CGFloat = 0.04 * ((value > brightness) ? 1 : -1)

        _brightessQueue.addOperations(stride(from: brightness, through: value, by: step).map({ [weak self] (value) -> Operation in
...
private let _brightessQueue: OperationQueue = {
